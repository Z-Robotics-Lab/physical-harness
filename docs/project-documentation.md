...
**响应大小硬闸**：storecli 的唯一出口 `_emit` 拒绝任何超过
`MAX_RESPONSE_BYTES`（4 MB）的响应体，改吐结构化的
`{error:"response too large", fn, bytes, limit, narrow}`——`narrow` 指名哪个参数
能收窄（如 `rsi_run` 的 `--round <n>`）。桥用固定缓冲区读 stdout，长过头的
face 必须**响亮地失败**，而不是被截断成一个静默的空面板（490 轮那次就是：
`rsi_series` 11 MB / `rsi_run` 22 MB 撞上 Node 默认的 1 MB maxBuffer，RSI 页
...
3. **agent 会话**：系统提示给出方法（一次一个假设；先在能解释现象的最高层定位——恢复/
   ...
   副本和 mission 卡的**代码大纲**（每个文件的类、方法、函数、常量及行号，6 KB 内；654 轮 54 次调用里 65 次 read/grep
   在重建这张图）、基线证据（每个种子的里程碑轨迹，靠 recovery 重试才通过的节点标出 fault 次数；每个 segment 结束时的
   **接触**——哪个物体在夹爪里、哪个在地上、底盘顶着哪个 fixture（`harness.media.read_contacts` 读 MuJoCo
   contact 表；4244 的罐子在 carry 段就掉到地上而 carry 自报成功，此前 40 轮全在修 drop）；首死节点的停滞几何，
   ...
   哪个动作的结果；小结果不折叠；关键帧图片只保留最新一条消息里的）。每条结果末尾带 harness 维护的
...
4. **验收**：`evaluate`（一轮最多 `max_evals`=3 次；`finish` 结束本轮，状态有变则先评一次）只接受**已有 probe 显示增益**的
   ...
   接受当且仅当**净增**（新通过的里程碑数多于退化的）且整任务成功的种子数**不减少**：用一个种子的
   一个中间里程碑换另一个种子的两个是进步，一个完成的种子换成另一个完成的种子也是，
   完成的种子变少则不是，不论换回多少中间里程碑。通过的评测**当场**成为 incumbent（副本快照到
